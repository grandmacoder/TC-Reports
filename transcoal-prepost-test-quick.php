...
function add_pre_post_test_quick(){
...
if(!current_user_can('manage_options')){
	...
	global $wpdb;
	$pre_post_test_quick = new PrePostTestFormQuick();

	?>
	<div class='wrap'>
                <div id='icon-users' class='icon32'></div>
                <h2>Quick Pre/Post Test Page</h2>
	<p><strong> This report does not retrieve answers to test questions.</strong></p>
	<form name="PrePostTestQuick" id="PrePostTestQuick">
	<span style="font-size: 16px; padding-right: 49px;"> <b>Module:</b> </span>
	<select name="module_list_data" id="module_list_data">
	<?php
	$course_list = $pre_post_test_quick->ModuleListDataQuick();
	...
	$pd_hub_user_group = $pre_post_test_quick->tc_pdhub_get_users_group_quick();
		...
		$user_rosters = $pre_post_test_quick->tc_pdhub_get_roster_leaders_quick($pd_hub_user->object_id);
			if($wpdb->num_rows > 0){
...
<p><strong>For Best Practices v1 tests, choose an end date  before Sept. 06, 2016</strong></p>
<span style="font-size: 16px; padding-right: 29px;"> <b>Start Date:</b> </span>	
 ...
 <INPUT TYPE="button" name="btnsubmitprepostquick" id="btnsubmitprepostquick" value="Download File"><div id="messageareaprepost" ></div>
 </form>
	</div>
<?php
 } 

class PrePostTestFormQuick{
	public function ModuleListDataQuick(){	
		global $wpdb;
		$course_list = $wpdb->get_results($wpdb->prepare("SELECT course_title, course_id FROM `wp_wpcw_courses` where course_opt_use_certificate ='use_certs' and course_id not in (38,39)", OBJECT));
		
		return $course_list;
	}
	public function tc_pdhub_get_users_group_quick(){
			global $wpdb;												
	...
	public function tc_pdhub_get_roster_leaders_quick($pd_hub_user_id){
			global $wpdb;
